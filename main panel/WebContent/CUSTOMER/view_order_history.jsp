					...
					<div class="container">
						<div class="table-responsive">
							...
							<%
										...
										List<BEAN_ORDER_FILE> list =ed.getAllInfo1(id);
										if(!list.isEmpty())
										{
							%>										
							<thead style="background-color:  #2874a6  !important;color:white;">
								...
								<% 
										for(BEAN_ORDER_FILE info: list)
										{
											...
											boolean status = DAO_ORDER_FILE.isValidUser(c_id,o_id);
									%> 
									...
									<tr style="background-color:  #e8e8e8 ;border-top: 2px solid black !important;font-weight: bolder !important;">
										...
										<form action="rate_file.jsp" method="post">
										
										<input type="hidden" name="trans_quote_id" value="<%=info.GET_TRANS_QUOTE_ID()%>" readonly>
										<%
												if(status)
												{ %>
															<input type="submit" name="action" value="Rate" class="form_control">
										<%
													}
												 else
												 {
												 %>
												 <input type="submit" name="action" value="no"  disabled>
												 <%
												 }
												 %>
