...
import COM.CUSTOMER.DAO.DAO_PROFILE;
@WebServlet("/CUSTOMER/PROFILE_ONE")
public class CONTROLLER_PROFILE_FILE extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException
	{
		...
		if(act.equalsIgnoreCase("edit"))
		{
			...
			req.getRequestDispatcher("LOGIN_CONTROL_CUSTOMER?page=Update Profile").forward(req,res);
		}	
		else if(act.equalsIgnoreCase("save"))
		{
			...
			req.getRequestDispatcher("LOGIN_CONTROL_CUSTOMER?page=View Profile").forward(req,res);
		}
		else if(act.equalsIgnoreCase("Change"))
		{
			...
			try
			{
				...
				if(rs.next())
				{
					check=two.GET_USER_PASSWORD();
					two.SET_USER_ID(rs.getInt("cust_id"));
				}
			}
			catch(Exception e)
			{	
			...
			one.SET_USER_ID(two.GET_USER_ID());
			DAO_PROFILE d1 = new DAO_PROFILE();
			...
			else
			{
			      ...
			      pw.println("function loa() \r\n" + 
			      		...
			      		"	.then((value) => {\r\n" + 
			      		"		window.location.href = \"LOGIN_CONTROL_CUSTOMER?page=Change Password\";\r\n" + 
			      		"	});\r\n" + 
			      		"}");
			      pw.println("</script>");
