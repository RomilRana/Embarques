...
public class DAO_TRANSPORTER_FILE {
	public void insertdata(BEAN_USER_FILE b2)
	{
		...
		{	
			...
			String Query = "insert into trans_table(first_name,last_name,trans_email,contact,trans_password,doi,dou) values(?,?,?,?,?,'"+dtf.format(now)+"','"+dtf.format(now)+"')";
	...
	public String getAllTransporter() 
	{
		...
		try
		{
			...
			while(re.next())
			{
				s1=re.getString("trans_email");
				email=s1.concat(",")+email;
			}
			re.close();
			ps.close();
		}
	...
	public List<COM.CUSTOMER.BEAN.BEAN_USER_FILE> getTransporter()
	{
		List<BEAN_USER_FILE> list = new ArrayList<BEAN_USER_FILE>();
